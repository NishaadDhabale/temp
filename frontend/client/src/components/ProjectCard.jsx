    ...
    <div className="bg-white rounded-lg shadow-md border border-gray-200 hover:shadow-lg transition-shadow overflow-hidden">
      ...
      <div className="px-6 py-4 space-y-3">
        <div className="flex items-center space-x-2 text-sm">
          ...
          <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full">{project.organization_type}</span>
        </div>
        <div className="flex items-center space-x-2 text-sm text-gray-600"><MapPin className="w-4 h-4 text-green-600" /><span>{project.location}</span></div>
        <div className="grid grid-cols-2 gap-3 pt-2">
          <div className="bg-blue-50 p-3 rounded-lg"><p className="text-xs text-blue-600 font-medium">Area</p><p className="text-lg font-bold text-blue-900">{project.area_hectares} ha</p></div>
          <div className="bg-green-50 p-3 rounded-lg"><p className="text-xs text-green-600 font-medium">Est. Carbon Credits</p><p className="text-lg font-bold text-green-900">{project.estimated_carbon_credits.toLocaleString()}</p></div>
        </div>
        <div className="flex items-center space-x-2 text-sm pt-2"><Leaf className="w-4 h-4 text-green-600" /><span className="text-gray-700 font-medium">{project.ecosystem_type}</span></div>
        <div className="text-xs text-gray-500 pt-2">Submitted: {new Date(project.submitted_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
      </div>
      ...
      {project.status === 'approved' && project.reviewed_at && (
        <div className="px-6 py-3 bg-gray-50 border-t border-gray-200">
          <p className="text-xs text-gray-600">
            Reviewed on {new Date(project.reviewed_at).toLocaleDateString('en-IN')} by {project.reviewed_by}
          </p>
        </div>
      )}
    </div>
